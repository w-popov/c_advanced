...
void draw_plots(struct AppProperties *ap, size_t nums_draw_plots,
  ...
  ScatterPlotSeries **series_array = (ScatterPlotSeries **)malloc(
      nums_draw_plots * sizeof(ScatterPlotSeries *));
  ...
  for (size_t i = 0; i < nums_draw_plots; ++i) {
    ...
    // цвет rgb
    series_array[i]->color =
        CreateRGBColor(plots[i].color.r, plots[i].color.g, plots[i].color.b);
  }
  ...
  int success =
      DrawScatterPlotFromSettings(imageReference, settings, &str_ref_err);
  ...
  if (success) {
    ...
    for (size_t i = 0; i < nums_draw_plots; ++i, heigt_title += 20) {
      ...
      RGBA *color_title =
          CreateRGBColor(plots[i].color.r, plots[i].color.g, plots[i].color.b);
      swprintf(title_buffer, 128, L"f%d = %ls", plots[i].fun_number,
               plots[i].title);
      DrawText(imageReference->image, 180, heigt_title, title_buffer,
               wcslen(title_buffer), color_title);
    }
    wchar_t *sign = L"cource work 2 (C adv) Popov V.G";
    DrawText(imageReference->image, 600, 15., sign, wcslen(sign),
    ...
    for (size_t i = 0; i < ap->size_arr_inters_points; ++i) {
      ...
      if (!isnan(ap->inters_points[i].root)) {
        ...
        DrawLine(imageReference->image, pixel_x, pixel_y_bottom, pixel_x,
                 pixel_y_top, thickness, line_color);

        RGBA *point_color = CreateRGBColor(1.0, 0.0, 0.0);
        double radius = 3.0;
        DrawCircle(imageReference->image, pixel_x, pixel_y_bottom, radius,
                   point_color);

        wchar_t root_buffer[32];
        swprintf(root_buffer, 32, L"xF%d=%.4f", ap->inters_points[i].id_F,
                 math_x);
        DrawText(imageReference->image, pixel_x - 35.0,
                 pixel_y_bottom + 25 + ladder_label, root_buffer,
                 wcslen(root_buffer), line_color);
      }
    }
...
struct AppProperties parse_json(const char *filename_json) 
{
  ...
  for (int i = 0; i < array_size; ++i) {
    ...
    app_props.plots_props_array[i].f =
        cJSON_IsNumber(f) ? (int)f->valuedouble : 0;
